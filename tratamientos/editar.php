...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $up=$conexion->prepare("UPDATE tratamientos SET paciente_id=?,diagnostico=?,plan=?,estado=?,fecha_inicio=?,fecha_fin=? WHERE id=?");
...
?>
<style>.form-page{display:flex;justify-content:center}.form-card{max-width:700px;width:100%;background:#fff;border-radius:10px;box-shadow:0 2px 8px rgba(0,0,0,.1)}.form-card-head{background:#0d6efd;color:#fff;padding:12px 16px;font-weight:700}.form-card-body{padding:16px}</style>
<div class="form-page">
  <div class="form-card">
        ...
        <div class="col-12">
          ...
          <select name="paciente_id" class="form-select">
            <?php foreach($pacientes as $p): ?>
              <option value="<?= $p['id'] ?>" <?= $p['id']==$t['paciente_id']?'selected':'' ?>><?= htmlspecialchars($p['nombre'].' '.$p['apellido']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-12">
          <label class="form-label">Diagnóstico</label>
          <input name="diagnostico" class="form-control" value="<?= htmlspecialchars($t['diagnostico']) ?>">
        </div>
        <div class="col-12">
          <label class="form-label">Plan</label>
          <textarea name="plan" class="form-control"><?= htmlspecialchars($t['plan']) ?></textarea>
        </div>
        <div class="col-md-6">
